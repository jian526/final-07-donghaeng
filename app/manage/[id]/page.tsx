...
export default function ManagePage() {
  ...
  useEffect(() => {
    ...
    const fetchManage = async () => {
      ...
      if (res.ok === 1) {
        // params의 [id]와 products[0]._id가 같은 신청자만 필터링
        const filtered = res.item.filter((item: Manage) => item.products[0]?._id === product_id);
        setManage(filtered);
      }
      setIsLoading(false);
    };
    ...
    if (result?.ok === 1) {
      ...
      if (isApprove) {
        if (productId && accessToken) {
          ...
          console.log('지금 몇인데 바이퀀티티', currentBuyQuantity);
    ...
    <>
      ...
      <DefaultLayout>
        <main className={styles['manage-body']}>
          ...
          {isLoading ? (
            <p className={styles['no-applicants']}>로딩중...</p>
