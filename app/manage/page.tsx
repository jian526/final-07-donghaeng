...
export default function ManagePage() {
  ...
  const [state, formAction] = useActionState(patchManage, { ok: 0, message: '' });
  const user = useUserStore((state) => state.user);
  const userId = user?._id;
  const accessToken = user?.token?.accessToken;
  console.log('유저아이디', userId);
  console.log('승인 결과', state);
          ...
          {manage && manage.length > 0 ? (
            manage.map((applicant) => (
              <div key={applicant._id} className={styles['applicant-div']}>
                <img src={applicant.user.image} alt="프로필이미지" className={styles['profile-img-desktop']} />
                <div className={styles['applicant-information']}>
                  <img src={applicant.user.image} alt="프로필이미지" className={styles['profile-img-mobile']} />
            ...
            <p className={styles['no-applicants']}>신청자가 없습니다.</p>
          )}
        </main>
      </DefaultLayout>
    </>
